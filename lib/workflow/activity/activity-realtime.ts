type WorkflowActivityRefetchListener = () => void

const listenersByWorkflowId = new Map<
  string,
  Set<WorkflowActivityRefetchListener>
>()

export function subscribeWorkflowActivityRefetch(
  workflowId: string,
  listener: WorkflowActivityRefetchListener
): () => void {
...
export function notifyWorkflowActivityRefetch(workflowId?: string): void {
  if (workflowId) {
